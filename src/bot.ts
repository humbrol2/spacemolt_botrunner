...
export class Bot {
    ...
    for (const n of notifications) {
      ...
      if (type === "system" && data && typeof data === "object") {
        ...
        if (d.damage !== undefined) {
          // Combat damage notification — log to bot's activity log
          ...
          // Faction alert on any damage event (cooldown prevents spam)
          const now = Date.now();
          ...
          // Update cached hull/shield from notification
          if (yourHull !== undefined) this.hull = yourHull;
          if (yourShield !== undefined) this.shield = yourShield;

        } else {
          // Generic system message — detect combat warnings
  ...
  private async sendCombatFactionAlert(
    ...
    try {
      // Fetch nearby players/pirates — query, no tick cost
      let nearbyInfo = "";
