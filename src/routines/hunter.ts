 ...
 *   - Brace  (shields critical): 0% damage dealt, shields regen 2× — use briefly to recover
...
/**
 * Pirates only exist in low-security and unregulated ("null") systems.
 * "high" and "medium" are too policed — don't bother patrolling them.
 * police_level is a 1-100 integer stored as a string: <=25 is huntable.
 */
function isHuntableSystem(securityLevel: string | undefined): boolean {
  ...
  // Match descriptive strings like "Low Security (slow police response)",
  // "Frontier (minimal police presence)", "Lawless (no police protection)"
  ...
  // Numeric police_level (1-100): <=25 means low/unregulated
...
/**
 * Find the nearest huntable system from a starting point.
 *
 * First does a BFS through stored connections (fast, respects graph structure).
 * Falls back to scanning all known systems in mapStore and using findRoute to
 * verify reachability — catches systems the BFS misses due to incomplete
 * connection data (e.g. connections only stored in one direction).
 */
function findNearestHuntableSystem(fromSystemId: string): string | null {
  ...
  // Phase 2: scan all known systems — catches systems not reachable via BFS
  // due to gaps in stored connection data (connections recorded in one direction only)
  for (const systemId of mapStore.getAllSystemIds()) {
    ...
    // Confirm there's an actual route before committing to it
...
/**
 * High and maximum-security systems are safe for docking, repairs, and insurance.
 * police_level >50 (i.e. "high" / "medium" and above) qualifies.
 */
function isSafeSystem(securityLevel: string | undefined): boolean {
  if (!securityLevel) return false;
  const level = securityLevel.toLowerCase().trim();

  // Match descriptive strings like "High Security (active police patrols)",
  // "Maximum Security (empire capital)"
...
/**
 * BFS to find the nearest high/medium-security system for safe docking.
 * Returns the system ID, or null if none is mapped.
 */
...
interface NearbyEntity {
  ...
  faction: string;
  isNPC: boolean;
  /** True when the entity was identified via a pirate_id field — definitely a target. */
  isPirate: boolean;
}
...
function parseNearby(result: unknown): NearbyEntity[] {
  ...
  return raw
    .map(e => {
      // Pirates use pirate_id rather than id/player_id
...
function isPirateTarget(entity: NearbyEntity, onlyNPCs: boolean): boolean {
  // Entities with a pirate_id are unambiguously pirate NPCs
...
/**
 * Navigate to a high-security system and dock for repairs/refuel/insurance.
 * High/max-security systems are policed and safe for station services.
 * Falls back to the nearest station in the current system if no safe system is mapped.
 */
...
/** Purchase ship insurance if not already covered and affordable. Requires docked. */
...
/**
 * Engage a single target to destruction or until forced to retreat.
 *
 * Combat sequence:
 *   1. Scan target for intel
 *   2. Attack to initiate battle
 *   3. Advance up to 3 zones toward engaged range
 *   4. Fire stance each tick; switch to Brace briefly when shields are critical
 *   5. Flee if hull drops to or below fleeThreshold
 *
 * Returns true if the target was destroyed/fled, false if we had to retreat.
 */
async function engageTarget(
  ...
  for (let zone = 0; zone < 3; zone++) {
    ...
    if (advResp.error) break; // already at engaged range or combat resolved
  }

  // Main combat loop — Fire stance with Brace when shields are critical
  ...
  for (let tick = 0; tick < MAX_COMBAT_TICKS; tick++) {
    ...
    // Brace briefly when shields are nearly gone and hull is hurting.
    // Brace = 0% damage dealt but shields regenerate at 2× — use sparingly.
    ...
    if (atkResp.error) {
      ...
      // Any of these mean the fight is over
      if (
...
/**
 * Find the next huntable system to move to after completing a patrol sweep.
 * Prefers lawless/null-sec systems over low-security — more pirates, better loot.
 * Falls back to any adjacent huntable system, then unmapped systems (may be huntable).
 */
function findNextHuntSystem(fromSystemId: string): string | null {
  ...
  // Priority 3: unmapped adjacent system — unknown security, worth scouting
...
export const hunterRoutine: Routine = async function* (ctx: RoutineContext) {
  ...
  while (bot.state === "running") {
    ...
    // Only use an explicitly configured system — homeSystem is the starting
    // position, not a patrol target. Without a setting, auto-discovery handles routing.
    ...
    // If a system is explicitly configured, go there.
    // Otherwise, check the current system's security level and move
    // out of policed space if needed.
    yield "find_patrol_system";

    if (patrolSystem && bot.system !== patrolSystem) {
      // User has a specific system configured — navigate there directly
      ...
      // Auto-select: ensure we're in a low/unregulated system
      await fetchSecurityLevel(ctx, bot.system);
      const currentSec = mapStore.getSystem(bot.system)?.security_level;

      if (!isHuntableSystem(currentSec)) {
        ctx.log("travel", `${bot.system} is ${currentSec || "unknown"} security — pirates only spawn in low/null systems. Searching for a huntable system...`);
        ...
        if (huntTarget) {
          ...
          // No known huntable system — jump to an unmapped adjacent system to scout
          ...
          if (target) {
            ...
            // Store this system's connections so the BFS can expand from here next cycle
    ...
    // ── Confirm we're actually in a huntable system before patrolling ──
    // This guards against the scouting fallback landing in a high-security system.
    await fetchSecurityLevel(ctx, bot.system);
    ...
    for (const poi of patrolPois) {
      ...
      if (targets.length === 0) {
        ...
        await scavengeWrecks(ctx); // loot any leftovers
        continue;
      }
      ...
      for (const target of targets) {
        ...
        if (won) {
          ...
          // Had to flee — end patrol immediately
    ...
    // ── Post-patrol decision: return to base or keep hunting? ──
    yield "post_patrol";
    await bot.refreshStatus();
    ...
    if (needsRepair || needsFuel) {
      // Hull or fuel too low — return to a high-security station for maintenance.
      ...
      // ── Post-patrol: missions, sell loot, insurance, refuel, repair ──
      ...
      // Hull and fuel are fine — keep hunting without returning to base.
      ctx.log("system", `Patrol sweep done — ${patrolKills} kill(s). Hull: ${postHull}% | Fuel: ${postFuel}% — continuing hunt...`);

      // In auto-discovery mode (no fixed patrol system), advance to an adjacent
      // huntable system — prefer lawless/null-sec over low-security.
      // In fixed-system mode, the loop head will navigate back to patrolSystem automatically.
      if (!patrolSystem) {
        ...
        if (nextSystem) {
          ...
          // Store new system's connections and security level for future cycles
          await getSystemInfo(ctx);
          await fetchSecurityLevel(ctx, bot.system);
        } else {
      ...
      // Loop continues — next cycle will patrol wherever we ended up
    }
  }
};
